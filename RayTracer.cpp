...
bool RayTracer::initialize() {
    ...
    glGenTextures(1, &m_imgAccumulationTexture);
    glBindTexture(GL_TEXTURE_2D, m_imgAccumulationTexture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, m_width, m_height, 0, GL_RGBA, GL_FLOAT, NULL);
    glBindImageTexture(1, m_imgAccumulationTexture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
...
void RayTracer::updateTriangles(const vector<Triangle>& triangles)
{
